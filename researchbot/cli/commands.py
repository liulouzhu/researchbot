...
from researchbot import __logo__, __version__
from researchbot.cli.stream import StreamRenderer, ThinkingSpinner
from researchbot.config.paths import get_workspace_path, is_default_workspace
...
app = typer.Typer(
    ...
    help=f"{__logo__} researchbot - Personal AI Research Assistant (internal package: nanobot)",
...
def _init_prompt_session() -> None:
    ...
    from researchbot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} researchbot[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} researchbot v{__version__} (internal package: nanobot)")
        raise typer.Exit()
...
def main(
    ...
    """researchbot - Personal AI Research Assistant."""
    pass
...
def onboard(
    ...
    """Initialize researchbot configuration and workspace."""
    from researchbot.config.loader import get_config_path, load_config, save_config, set_config_path
    from researchbot.config.schema import Config
    ...
    if wizard:
        from researchbot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'researchbot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'researchbot agent -m "Hello!"'
    gateway_cmd = "researchbot gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} researchbot is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from researchbot.channels.registry import discover_all
...
def _make_provider(config: Config):
    ...
    """
    from researchbot.providers.base import GenerationSettings
    ...
    if backend == "openai_codex":
        from researchbot.providers.openai_codex_provider import OpenAICodexProvider
        provider = OpenAICodexProvider(default_model=model)
    elif backend == "azure_openai":
        from researchbot.providers.azure_openai_provider import AzureOpenAIProvider
        ...
        from researchbot.providers.anthropic_provider import AnthropicProvider
        ...
        from researchbot.providers.openai_compat_provider import OpenAICompatProvider
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from researchbot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show researchbot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    from researchbot.api.server import create_app
    from researchbot.bus.queue import MessageBus
    from researchbot.session.manager import SessionManager
    ...
    agent_loop = AgentLoop(
        ...
        literature_config=runtime_config.literature,
    )
...
def gateway(
    ...
    """Start the researchbot gateway."""
    from researchbot.agent.loop import AgentLoop
    from researchbot.bus.queue import MessageBus
    from researchbot.channels.manager import ChannelManager
    from researchbot.cron.service import CronService
    from researchbot.cron.types import CronJob
    from researchbot.heartbeat.service import HeartbeatService
    from researchbot.session.manager import SessionManager
    ...
    console.print(f"{__logo__} Starting researchbot gateway version {__version__} on port {port}...")
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from researchbot.utils.evaluator import evaluate_response
        ...
        if job.payload.deliver and job.payload.to and response:
            ...
            if should_notify:
                from researchbot.bus.events import OutboundMessage
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show researchbot runtime logs during chat"),
):
    ...
    if message:
        ...
        from researchbot.bus.events import InboundMessage
...
def channels_status():
    """Show channel status."""
    from researchbot.channels.registry import discover_all
    from researchbot.config.loader import load_config
...
def _get_bridge_dir() -> Path:
    ...
    from researchbot.config.paths import get_bridge_install_dir
...
def plugins_list():
    ...
    from researchbot.channels.registry import discover_all, discover_channel_names
    from researchbot.config.loader import load_config
...
@app.command()
def status():
    """Show researchbot status."""
    from researchbot.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} researchbot Status\n")
